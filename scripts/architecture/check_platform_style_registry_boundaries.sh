...
check_no_unapproved_matches \
  ...
  'apps/Studio/Tools/CustomizationStudio/Services/VisualCustomizerApprovalRequestService\.php:[0-9]+:(use Apps\\Platform\\StyleRegistry\\Services\\ApprovedStyleRegistry;|require_once APP_ROOT \. '\''/apps/Platform/StyleRegistry/Contracts/ApprovedStyleRegistryContract\.php'\'';|require_once APP_ROOT \. '\''/apps/Platform/StyleRegistry/Services/ApprovedStyleRegistry\.php'\'';|[[:space:]]*\$registry = new ApprovedStyleRegistry\(\);)|Tests/[^:]+:[0-9]+:[[:space:]]*fwrite[[:space:]]*\([[:space:]]*STDERR' \
  "${customization_runtime_files[@]}"
