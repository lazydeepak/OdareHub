-- Bom module contract (Manufacturing-owned BOM/Recipe foundation).
-- Co-declaration of manufacturing_bom / manufacturing_bom_line first introduced by
-- Products/migrations/013_add_manufacturing_bom.sql. Identical DDL on purpose:
-- (plugin, filename) migration tracking makes both copies idempotent; keep in sync.
-- finished_item_ref / component_item_ref are OPAQUE canonical item references
-- (docs/shared-items-foundation/canonical-item-contract.md); no FK to Shared identity.
