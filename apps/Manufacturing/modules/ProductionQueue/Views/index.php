...
<div class="card u-style-d4645e97d7">
  <div class="ui-block">
    <h2 class="u-style-1169661891"><?= e(t('mfg.production_queue.title')) ?></h2>
    <div class="muted u-style-9b125dac7d">
      <?php if ($isToday): ?><?= e(t('mfg.production_queue.today')) ?> &mdash; <?= e(date('l, d M Y', strtotime($date))) ?>
      <?php elseif ($isPast): ?><span class="u-style-8ce6e50b5f"><?= e(t('mfg.production_queue.past')) ?> &mdash; <?= e(date('d M Y', strtotime($date))) ?></span>
      <?php else: ?><span class="u-style-7b1b003e7f"><?= e(t('mfg.production_queue.upcoming')) ?> &mdash; <?= e(date('d M Y', strtotime($date))) ?></span>
      <?php endif; ?>
    </div>
  </div>
  <div class="u-style-58703f152a">
    ...
    <a class="btn" href="/apps/manufacturing/production-queue"><?= $isToday ? e(t('mfg.production_queue.today')) : e(t('mfg.production_queue.go_to_today')) ?></a>
    ...
    <a class="btn ok" href="/production-plans/add">+ <?= e(t('mfg.production_queue.plan')) ?></a>
    <a class="btn" href="/production-entries/add">+ <?= e(t('mfg.production_queue.entry')) ?></a>
    <a class="btn" href="/production-plans"><?= e(t('mfg.production_queue.list_view')) ?></a>
  </div>
</div>

<!-- Filter -->
<div class="card">
  <form method="get" action="/apps/manufacturing/production-queue" class="queue-filter-form">
    <div class="queue-filter-row">
      <div class="queue-filter-field">
        <label class="muted u-style-81fb9ffd75"><?= e(t('common.date')) ?></label>
        <input class="input" type="date" name="date" value="<?= e($date) ?>">
      </div>
      <div class="queue-filter-field">
        <label class="muted u-style-81fb9ffd75"><?= e(t('common.machine')) ?></label>
        <select name="machine_id">
          <option value="0"><?= e(t('mfg.production_queue.all_machines')) ?></option>
    ...
    <div class="queue-filter-actions">
      ...
      <a class="btn" href="/apps/manufacturing/production-queue"><?= e(t('common.reset')) ?></a>
    </div>
  </form>
</div>
...
<div class="stat-row">
  <div class="stat-box">
    <div class="stat-box-label"><?= e(t('mfg.production_queue.plans')) ?></div>
    <div class="stat-box-value"><?= count($rows) ?></div>
  </div>
  <div class="stat-box">
    <div class="stat-box-label"><?= e(t('mfg.production_queue.total_planned_qty')) ?></div>
    <div class="stat-box-value"><?= number_format($totalPlanned, 0) ?></div>
  </div>
  <div class="stat-box">
    <div class="stat-box-label"><?= e(t('mfg.production_queue.produced_good')) ?></div>
    <div class="stat-box-value" style="color: var(--text)"><?= number_format($totalProduced, 0) ?></div>
  </div>
  <div class="stat-box">
    <div class="stat-box-label"><?= e(t('mfg.production_queue.overall_progress')) ?></div>
  ...
  <?php if ($isToday): ?>
  <div class="stat-box">
    <div class="stat-box-label"><?= e(t('mfg.production_queue.time_of_day')) ?></div>
    ...
    <div class="u-style-a4abb89f4d"><?= round(24 - $nowHour, 1) ?><?= e(t('mfg.production_queue.hours_remaining_suffix')) ?></div>
  </div>
  <?php endif; ?>
</div>

<?php if (empty($rows)): ?>
<div class="card u-style-4f60c780bb"><?= e(t('mfg.production_queue.no_plans_for_date')) ?><?= $machine_id > 0 ? ' / ' . e(t('common.machine')) : '' ?>.</div>
<?php else: ?>
...
<?php foreach ($rows as $r):
  ...
  if ($pct >= 100) {
    ...
    $badgeLabel = t('mfg.production_queue.done');
  } elseif ($isPast && $pct < 100) {
    $cardClass  = 'overdue';
    $badgeClass = 'badge-overdue';
    $badgeLabel = t('mfg.production_queue.overdue');
  } elseif ($pct > 0) {
    $cardClass  = 'on-track';
    $badgeClass = 'badge-progress';
    $badgeLabel = t('mfg.production_queue.in_progress');
  } elseif ($isFuture) {
    $cardClass  = '';
    $badgeClass = 'badge-upcoming';
    $badgeLabel = t('mfg.production_queue.upcoming');
  } else {
    $cardClass  = '';
...
<div class="queue-card <?= $cardClass ?>">
  ...
  <div class="queue-card-header">
    ...
    <span class="queue-badge <?= $badgeClass ?>"><?= e((string)$badgeLabel) ?></span>
  </div>

  <!-- Progress bar -->
  <div class="ui-block">
    <div class="u-style-edeed852ef">
      <span class="u-style-57f9e31d78"><?= e(t('mfg.production_queue.production_progress')) ?></span>
  ...
  <div class="queue-nums">
    <div class="queue-num">
      <div class="queue-num-label"><?= e(t('mfg.production_queue.planned')) ?></div>
      <div class="queue-num-value"><?= number_format($planned, 0) ?></div>
    </div>
    <div class="queue-num">
      <div class="queue-num-label"><?= e(t('mfg.production_queue.produced')) ?></div>
      <div class="queue-num-value u-style-ce8274176b"><?= number_format($produced, 0) ?></div>
    </div>
    <?php if ($rejected > 0): ?>
    <div class="queue-num">
      <div class="queue-num-label"><?= e(t('mfg.production_queue.rejected')) ?></div>
      <div class="queue-num-value u-style-8ce6e50b5f"><?= number_format($rejected, 0) ?></div>
    </div>
    <?php endif; ?>
    <div class="queue-num">
      <div class="queue-num-label"><?= e(t('mfg.production_queue.remaining')) ?></div>
      <div class="queue-num-value" style="color: var(--text)"><?= number_format($remaining, 0) ?></div>
    </div>
    <?php if ($cyclePerUnit !== null): ?>
    <div class="queue-num">
      <div class="queue-num-label"><?= e(t('mfg.production_queue.cycle_min_unit')) ?></div>
  ...
  <div class="ui-block">
    <div class="u-style-0899906b77">
      <span><?= e(t('mfg.production_queue.planned_time')) ?></span>
  ...
  <div class="u-style-472fd3529f">
    <span><?= e(t('mfg.production_queue.plan_prefix')) ?> <?= e((string)$r['plan_date']) ?></span>
    <?php if (!$isToday): ?>
      <?php
        $diffDays = (int)((strtotime($date) - strtotime($today)) / 86400);
        $diffLabel = $diffDays > 0
            ? sprintf((string)t('module.production_queue.days_from_today'), $diffDays)
            : sprintf((string)t('module.production_queue.days_ago'), abs($diffDays));
      ?>
  ...
  <div class="queue-actions">
    <a class="btn" href="/production-entries/add?product_id=<?= (int)$r['product_id'] ?>" style="font-size:.8em;padding:4px 10px"><?= e(t('module.production_queue.add_entry')) ?></a>
    <a class="btn" href="/production-plans/edit?id=<?= (int)$r['id'] ?>" style="font-size:.8em;padding:4px 10px"><?= e(t('mfg.production_queue.edit_plan')) ?></a>
    <a class="btn" href="/production-entries?machine_id=<?= (int)$r['machine_id'] ?>&production_date=<?= urlencode($r['plan_date']) ?>" style="font-size:.8em;padding:4px 10px"><?= e(t('mfg.production_queue.entries')) ?></a>
  </div>
</div>
<?php endforeach; ?>
</div>
