...
namespace Generated\ManufacturingApp\Providers;

final class ProductionPlanProvider
{
    ...
    {
        return rtrim((string)(defined('APP_ROOT') ? APP_ROOT : dirname(__DIR__, 5)), '/') . '/storage/appstudio/generated_data/manufacturing_app/production_plan.json';
    }
    ...
    private static function writeRows(array $rows): bool
    {
        ...
        $payload = ['schema_version' => 'studio.generated-data.v1', 'app_key' => 'manufacturing_app', 'module_key' => 'production_plan', 'updated_at' => gmdate('c'), 'rows' => array_values($rows)];
        $json = json_encode($payload, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        if ($json === false || json_decode($json, true) === null) {
            return false;
        }
        $tmp = $dir . '/.production_plan.' . bin2hex(random_bytes(4)) . '.tmp';
