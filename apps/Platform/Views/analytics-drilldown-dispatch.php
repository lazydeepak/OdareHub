<?php
/**
 * Dispatch Status Drill-down View
 * Displays dispatch entries with shipment details
 */
...
<head>
    ...
    <title><?php echo $this->tr('analytics.drilldown.dispatch.title'); ?></title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="header">
            <h1><?php echo $this->tr('analytics.drilldown.dispatch.title'); ?></h1>
        ...
        <?php if (!empty($drilldown['entries'])): ?>
            <div class="table-wrapper">
                <table>
                    <thead>
                        <tr>
                            <th><?php echo $this->tr('analytics.drilldown.dispatch.id'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.dispatch.date'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.dispatch.qty'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.dispatch.destination'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.dispatch.type'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.dispatch.status'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.dispatch.cases'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.dispatch.pallets'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.dispatch.dispatched_at'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($drilldown['entries'] as $entry): ?>
                            ...
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars((string)($entry['dispatch_date'] ?? ''), ENT_QUOTES); ?></td>
                                <td class="qty-numeric"><?php echo number_format((float)($entry['dispatchable_qty'] ?? 0), 2); ?></td>
                                <td><?php echo htmlspecialchars((string)($entry['destination'] ?? ''), ENT_QUOTES); ?></td>
                                <td><?php echo htmlspecialchars((string)($entry['dispatch_type'] ?? ''), ENT_QUOTES); ?></td>
                                <td>
                                    <span class="status-badge <?php echo htmlspecialchars((string)($entry['status_class'] ?? ''), ENT_QUOTES); ?>">
                                        <?php echo ucfirst((string)($entry['dispatch_status'] ?? '')); ?>
                                    </span>
                                </td>
                                <td class="qty-numeric"><?php echo (int)($entry['cases_count'] ?? 0); ?></td>
                                <td class="qty-numeric"><?php echo (int)($entry['pallets_count'] ?? 0); ?></td>
                                <td><?php echo $entry['dispatched_at'] ? date('M d, Y H:i', strtotime((string)($entry['dispatched_at'] ?? ''))) : '—'; ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
