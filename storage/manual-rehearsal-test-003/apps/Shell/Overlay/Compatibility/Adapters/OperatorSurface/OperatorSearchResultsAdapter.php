...
final class OperatorSearchResultsAdapter
{
...
(function () {
  ...
  window.SusankhyaOS.ShellOverlayAdapters.createOperatorSearchResultsAdapter = function (options) {
    var config = options && typeof options === 'object' ? options : {};
    var overlayType = 'operator_search_results';
    var instance = null;
    var openState = false;
    ...
    function open(reason, state) {
      ...
      if (fw && fw.manager && typeof fw.manager.open === 'function') {
        instance = fw.manager.open({
          ...
          reason: reason || 'open',
          state: state || 'results',
          triggerId: config.triggerId || 'headerRouteSearch',
          surfaceId: config.surfaceId || 'operatorSearchResults',
          owner: 'Shell',
          isModal: false,
          preset: 'dropdown',
          escape: false,
    ...
    function syncState(state, reason) {
      ...
      return close(reason || normalized || 'hidden');
    }
