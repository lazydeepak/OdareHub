<?php
// Operator Layer View: fulfillment (unified Preparation + Dispatch)
// All composer scope variables available via include.
?>
<?php
    $preparation = (array)($data['preparation_focus'] ?? []);
    $readyRows = (array)($preparation['ready_rows'] ?? []);
    $readyIndex = (array)($preparation['ready_index'] ?? []);
    $bundleRows = (array)($preparation['bundle_rows'] ?? []);
    $preparationDate = (string)($preparation['selected_date'] ?? date('Y-m-d'));
    $preparationProductId = (int)($preparation['selected_product_id'] ?? 0);
    $preparationDestinations = (array)($preparation['destinations'] ?? []);
    $preparationDestinationDefaults = (array)($preparation['destination_defaults'] ?? []);
    $preparationDefaultDriver = trim((string)($preparation['default_driver_name'] ?? ''));
    $preparationDefaultTruck = trim((string)($preparation['default_truck_no'] ?? ''));
    $preparationFlash = trim((string)($preparation['flash'] ?? ''));
    $preparationError = trim((string)($preparation['error'] ?? ''));

    $dispatch = (array)($data['dispatch_focus'] ?? []);
    $dispatchRows = (array)($dispatch['rows'] ?? []);
    $dispatchProducts = (array)($dispatch['products'] ?? []);
    $dispatchDestinations = (array)($dispatch['destinations'] ?? []);
    $dispatchDate = (string)($dispatch['selected_date'] ?? date('Y-m-d'));
    $dispatchToDate = (string)($dispatch['selected_to_date'] ?? $dispatchDate);
    $dispatchProductId = (int)($dispatch['selected_product_id'] ?? 0);
    $dispatchDefaultDriver = trim((string)($dispatch['default_driver_name'] ?? ''));
    $dispatchDefaultTruck = trim((string)($dispatch['default_truck_no'] ?? ''));
    $dispatchFlash = trim((string)($dispatch['flash'] ?? ''));
    $dispatchError = trim((string)($dispatch['error'] ?? ''));

    $currentQuery = (array)($data['current_query'] ?? []);
    $activeTab = trim((string)($currentQuery['tab'] ?? 'prepare'));
    if (!in_array($activeTab, ['prepare', 'dispatch'], true)) {
        $activeTab = 'prepare';
    }
    $uUser = urlencode((string)($data['username'] ?? ''));
    $fulfillmentBase = '/u/' . $uUser . '/fulfillment';
?>
<section class="recent-focus" aria-label="<?php echo htmlspecialchars($this->tr('operator.fulfillment.title', 'Fulfillment')); ?>">
    <h2 class="dashboard-top-title"><?php echo htmlspecialchars($this->tr('operator.fulfillment.title', 'Fulfillment')); ?></h2>
    <p class="recent-focus-subtitle"><?php echo htmlspecialchars($this->tr('operator.fulfillment.subtitle', 'Manage preparation bundles and dispatch queue in one place.')); ?></p>

    <!-- Sub-nav tabs (pill style matching production/processing) -->
    <nav class="operator-subnav" aria-label="<?php echo htmlspecialchars($this->tr('operator.fulfillment.tabs.label', 'Fulfillment sections')); ?>">
        <a
            href="<?php echo htmlspecialchars($fulfillmentBase . '?tab=prepare'); ?>"
            class="operator-subnav-item<?php echo $activeTab === 'prepare' ? ' operator-subnav-item--active' : ''; ?>"
            <?php echo $activeTab === 'prepare' ? 'aria-current="page"' : ''; ?>
        ><?php echo htmlspecialchars($this->tr('operator.fulfillment.tab.prepare', 'Preparation')); ?></a>
        <a
            href="<?php echo htmlspecialchars($fulfillmentBase . '?tab=dispatch'); ?>"
            class="operator-subnav-item<?php echo $activeTab === 'dispatch' ? ' operator-subnav-item--active' : ''; ?>"
            <?php echo $activeTab === 'dispatch' ? 'aria-current="page"' : ''; ?>
        ><?php echo htmlspecialchars($this->tr('operator.fulfillment.tab.dispatch', 'Dispatch')); ?></a>
    </nav>

    <?php if ($activeTab === 'prepare'): ?>
        ...
        <section class="surface-card" aria-label="<?php echo htmlspecialchars($this->tr('operator.preparation.form.title', 'Prepare Order')); ?>">
            ...
            <div class="production-focus-actions u-mb-10">
                <button
                    ...
                    aria-expanded="false"
                ><?php echo htmlspecialchars($this->tr('operator.preparation.form.create', 'Prepare Order')); ?></button>
            </div>
            <div class="ui-block" id="preparationCreatePanel" hidden>
                <form method="post" action="/u/<?php echo $uUser; ?>/preparation/bundle/create" class="production-focus-form" id="preparationReadyForm">
                    <input type="hidden" name="csrf" value="<?php echo htmlspecialchars((string)\App\Core\Auth::csrfToken()); ?>">
                    <input type="hidden" name="redirect_to" value="<?php echo htmlspecialchars($fulfillmentBase . '?tab=prepare'); ?>">
    ...
    <?php elseif ($activeTab === 'dispatch'): ?>
        ...
        <section class="surface-card" aria-label="<?php echo htmlspecialchars($this->tr('operator.dispatch.form.title', 'Dispatch Entry')); ?>">
            ...
            <form method="post" action="/u/<?php echo $uUser; ?>/dispatch/create" class="production-focus-form">
                <input type="hidden" name="csrf" value="<?php echo htmlspecialchars((string)\App\Core\Auth::csrfToken()); ?>">
                <input type="hidden" name="redirect_to" value="<?php echo htmlspecialchars($fulfillmentBase . '?tab=dispatch'); ?>">
            ...
            <div class="recent-focus-table-wrap operator-table-scroll">
                <table class="recent-focus-table">
                    ...
                    <tbody>
                        <?php if ($dispatchRows === []): ?>
                            <tr>
                                <td colspan="8"><?php echo htmlspecialchars($this->tr('operator.dispatch.table.empty', 'No dispatch entries found in current window.')); ?></td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($dispatchRows as $row): ?>
                                <?php $bundleId = (int)($row['bundle_id'] ?? 0); ?>
                                <tr
                                    <?php if ($bundleId > 0): ?>
                                    class="operator-clickable-row"
                                    data-href="/u/<?php echo $uUser; ?>/dispatch/detail?bundle_id=<?php echo $bundleId; ?>"
                                    title="<?php echo htmlspecialchars($this->tr('operator.dispatch.table.open_detail', 'Open detail')); ?>"
                                    <?php endif; ?>
                                >
                                    <td><?php echo htmlspecialchars((string)($row['dispatch_date'] ?? '')); ?></td>
                                    <td><?php echo htmlspecialchars((string)($row['destination'] ?? '-')); ?></td>
                                    <td><?php echo htmlspecialchars((string)($row['part_name'] ?? '-')); ?></td>
