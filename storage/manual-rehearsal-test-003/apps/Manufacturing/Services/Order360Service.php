...
final class Order360Service
{
    ...
    {
        $refs = [
            ['entity_type' => 'daily_order', 'entity_id' => $dailyOrderId],
        ];

        foreach ($productionEntries as $row) {
            $id = (int)($row['id'] ?? 0);
            if ($id > 0) {
                $refs[] = ['entity_type' => 'production_entry', 'entity_id' => $id];
            }
        }
        foreach ($qcEntries as $row) {
            $id = (int)($row['id'] ?? 0);
            if ($id > 0) {
                $refs[] = ['entity_type' => 'qc_entry', 'entity_id' => $id];
            }
        }
        foreach ($dispatchEntries as $row) {
            $id = (int)($row['id'] ?? 0);
            if ($id > 0) {
                $refs[] = ['entity_type' => 'dispatch_entry', 'entity_id' => $id];
            }
        }

        return HandoffEngine::summaryForChain($refs);
    }
