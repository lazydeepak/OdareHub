<?php
/**
 * QC Performance Drill-down View
 * Displays QC entries with detailed quality metrics
 */
...
<head>
    ...
    <title><?php echo $this->tr('analytics.drilldown.qc.title'); ?></title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .pagination .disabled:hover { background: var(--card-bg); border-color: var(--border); color: var(--text); }
...
<body>
    <div class="container">
        ...
        <div class="header">
            <h1><?php echo $this->tr('analytics.drilldown.qc.title'); ?></h1>
        ...
        <?php if (!empty($drilldown['entries'])): ?>
            <div class="table-wrapper">
                <table>
                    <thead>
                        <tr>
                            <th><?php echo $this->tr('analytics.drilldown.qc.id'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.qc.type'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.qc.checked_qty'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.qc.pass_qty'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.qc.fail_qty'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.qc.pass_rate'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.qc.status'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.qc.approval_status'); ?></th>
                            <th><?php echo $this->tr('analytics.drilldown.qc.created_at'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($drilldown['entries'] as $entry): ?>
                            ...
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars((string)($entry['qc_type'] ?? ''), ENT_QUOTES); ?></td>
                                <td class="qty-numeric"><?php echo number_format((float)($entry['checked_qty'] ?? 0), 2); ?></td>
                                <td class="qty-numeric"><?php echo number_format((float)($entry['pass_qty'] ?? 0), 2); ?></td>
                                <td class="qty-numeric"><?php echo number_format((float)($entry['fail_qty'] ?? 0), 2); ?></td>
                                <td class="<?php echo htmlspecialchars((string)($entry['pass_rate'] ?? 0) >= 95 ? 'status-good' : ((float)($entry['pass_rate'] ?? 0) >= 85 ? 'status-warn' : 'status-danger'), ENT_QUOTES); ?>">
                                    <?php echo htmlspecialchars((string)($entry['pass_rate_text'] ?? '0%'), ENT_QUOTES); ?>
                                </td>
                                <td>
                                ...
                                <td>
                                    <span class="status-badge <?php echo htmlspecialchars((string)($entry['approval_class'] ?? ''), ENT_QUOTES); ?>">
                                        <?php echo ucfirst((string)($entry['approval_status'] ?? '')); ?>
                                    </span>
                                </td>
                                <td><?php echo date('M d, Y', strtotime((string)($entry['created_at'] ?? ''))); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
