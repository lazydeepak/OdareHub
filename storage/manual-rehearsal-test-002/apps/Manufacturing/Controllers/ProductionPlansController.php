...
require_once APP_ROOT . '/apps/Manufacturing/modules/ProductionPlans/ProductionPlanService.php';
...
final class ProductionPlansController
{
    public static function index(View $view): void
    {
        ...
        $auditSummary = AuditLogService::latestByEntityIds(WorkflowPolicy::MODULE_PRODUCTION_PLAN, array_map(static fn(array $row): int => (int)($row['id'] ?? 0), $rows));

        $view->render('manufacturing::production_plans/index.php', [
    ...
    public static function addForm(View $view): void
    {
        ...
        $view->render('manufacturing::production_plans/add.php', [
    ...
    public static function create(array $input): void
    {
        ...
        $recordId = ProductionPlanService::create($data, $context);

        $created = DB::fetchOne('SELECT * FROM production_plans WHERE id=? LIMIT 1', [$recordId]) ?: [];
        AuditLogService::logEvent(
            WorkflowPolicy::MODULE_PRODUCTION_PLAN,
            $recordId,
    ...
    public static function createDraft(array $input): void
    {
        ...
        DB::query(
            'INSERT INTO production_plans (plan_date, machine_id, product_id, planned_qty, sequence_no, runtime, status, plan_type, reference_doctype, reference_name, coverage_pct, shortage_qty, auto_created, notes, added_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
            [
                $planDate,
                $machineId,
                $productId,
        ...
        $draftId = (int)DB::conn()->insert_id;
        $created = DB::fetchOne('SELECT * FROM production_plans WHERE id=? LIMIT 1', [$draftId]) ?: [];
        AuditLogService::logEvent(
            WorkflowPolicy::MODULE_PRODUCTION_PLAN,
            $draftId,
            AuditLogService::EVENT_LIFECYCLE,
            ...
            [
                ...
                'module' => 'production',
                'new_state' => (string)($created['status'] ?? 'draft'),
                'note' => 'Draft production plan created from coverage flow.',
    ...
    public static function editForm(View $view, int $id): void
    {
        ...
        AuditLogService::ensureSchema();
        self::ensureAddedByColumn();
        HandoffEngine::syncProductionPlan($id);
        ...
        $view->render('manufacturing::production_plans/edit.php', [
            ...
            'ownership_summary' => HandoffEngine::summaryForEntity('production_plan', $id),
            'activity_timeline' => AuditLogService::timeline(WorkflowPolicy::MODULE_PRODUCTION_PLAN, $id, 80),
        ]);
    }
    ...
    public static function update(array $input): void
    {
        ...
        if (!$canEditPlan) {
            ...
            DB::query(
                'UPDATE production_plans SET added_by=?, updated_at=NOW() WHERE id=?',
                [$requestedAddedBy, $id]
            );
            $afterAddedBy = DB::fetchOne('SELECT * FROM production_plans WHERE id=? LIMIT 1', [$id]) ?: [];
            AuditLogService::logEvent(
                WorkflowPolicy::MODULE_PRODUCTION_PLAN,
                $id,
                AuditLogService::EVENT_ASSIGNMENT,
                AuditLogService::ACTION_ASSIGNED,
                $currentUser,
                [
                    'app' => 'manufacturing',
                    'module' => 'production',
                    'note' => 'Added by reassigned.',
                    'diff' => AuditLogService::diffImportantFields($existing, $afterAddedBy, ['added_by']),
                ]
            );
        ...
        DB::query(
            'UPDATE production_plans SET plan_date=?, machine_id=?, product_id=?, planned_qty=?, sequence_no=?, runtime=?, status=?, plan_type=?, reference_doctype=?, reference_name=?, coverage_pct=?, shortage_qty=?, auto_created=?, notes=?, added_by=?, updated_at=NOW() WHERE id=?',
            [
                $data['plan_date'], $data['machine_id'], $data['product_id'], $data['planned_qty'], $data['sequence_no'],
                $data['runtime'], $data['status'], $data['plan_type'], $data['reference_doctype'], $data['reference_name'],
                $data['coverage_pct'], $data['shortage_qty'], $data['auto_created'], $data['notes'], $requestedAddedBy, $id
            ]
        );

        $after = DB::fetchOne('SELECT * FROM production_plans WHERE id=? LIMIT 1', [$id]) ?: [];
        AuditLogService::logEvent(
            WorkflowPolicy::MODULE_PRODUCTION_PLAN,
            $id,
            AuditLogService::EVENT_LIFECYCLE,
    ...
    public static function delete(int $id): void
    {
        ...
        DB::query('DELETE FROM production_plans WHERE id=? LIMIT 1', [$id]);
        AuditLogService::logEvent(
            WorkflowPolicy::MODULE_PRODUCTION_PLAN,
            $id,
            AuditLogService::EVENT_LIFECYCLE,
    ...
    public static function approvalAction(array $input): void
    {
        ...
        $supervisoryActions = ['approve', 'reject', 'hold', 'resume', 'finalize', 'cancel'];
        if ($authorityRole === 'app_user' && (in_array($action, $supervisoryActions, true) || $rawAction === 'unlock_override')) {
            self::flash('err', 'Supervisory workflow actions require app admin or platform admin access.');
            header('Location: ' . $redirect);
            exit;
        }

        $existing = WorkflowTransitionEngine::fetchRecord(WorkflowPolicy::MODULE_PRODUCTION_PLAN, $id);
        if (!$existing) {
            self::flash('err', 'Production plan not found.');
            header('Location: /production-plans');
            exit;
        ...
        if ($rawAction === 'unlock_override') {
            ...
            if (!WorkflowPolicy::canOverrideLock(WorkflowPolicy::MODULE_PRODUCTION_PLAN, $currentUser)
                || !WorkflowPolicy::isLocked(WorkflowPolicy::MODULE_PRODUCTION_PLAN, $existingWithDefaults)) {
        ...
        if (($action === 'approve' || $action === 'finalize') && ($validationError = WorkflowPolicy::validateForApproval(WorkflowPolicy::MODULE_PRODUCTION_PLAN, $existing)) !== null) {
    ...
    public static function printPdf(int $id, bool $download): void
    {
        ...
        $html = self::renderPdfTemplate('manufacturing::production_plans/pdf_production_plan.php', ['row' => $row]);
    ...
    public static function printNextTwoWeeks(View $view): void
    {
        ...
        $view->render('manufacturing::production_plans/print_next_two_weeks.php', [
    ...
    public static function printNextTwoWeeksPdf(): void
    {
        ...
        $report = self::nextTwoWeeksReportData();
        $html = self::renderPdfTemplate('manufacturing::production_plans/pdf_next_two_weeks.php', [
            'start_date' => $report['start_date'],
            'end_date' => $report['end_date'],
            'total_rows' => $report['total_rows'],
            'groups' => $report['groups'],
