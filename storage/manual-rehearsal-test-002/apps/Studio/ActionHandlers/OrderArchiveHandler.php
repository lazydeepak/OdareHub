...
final class OrderArchiveHandler implements StudioActionHandler
{
    /**
    ...
    {
        if (!StudioAuthorizationService::can('orders', 'archive', $context)) {
        ...
        try {
            return $this->withTransaction(function () use ($appKey, $context, $orderNo): array {
                ...
                $affected = $repo->archive(['order_no' => $orderNo]);
                return ['ok' => $affected > 0, 'code' => $affected > 0 ? 'archived' : 'lifecycle_blocked', 'message' => $affected > 0 ? 'archived' : 'lifecycle_blocked', 'errors' => [], 'data' => ['affected' => $affected, 'order_no' => $orderNo]];
            });
        } catch (\Throwable) {
