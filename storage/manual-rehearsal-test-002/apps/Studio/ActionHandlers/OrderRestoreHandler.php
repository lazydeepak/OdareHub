...
final class OrderRestoreHandler implements StudioActionHandler
{
    /**
    ...
    {
        if (!StudioAuthorizationService::can('orders', 'restore', $context)) {
        ...
        try {
            return $this->withTransaction(function () use ($appKey, $context, $orderNo): array {
                ...
                $affected = $repo->restore(['order_no' => $orderNo]);
                return ['ok' => $affected > 0, 'code' => $affected > 0 ? 'restored' : 'lifecycle_blocked', 'message' => $affected > 0 ? 'restored' : 'lifecycle_blocked', 'errors' => [], 'data' => ['affected' => $affected, 'order_no' => $orderNo]];
            });
        } catch (\Throwable) {
